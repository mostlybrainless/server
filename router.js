    ...
    if(docs.length > 0){
    	...
    	response.end();
  		...
  		if(docs.length > 0){
  			db.close();
  			response.writeHead(403, {"Content-Type": "text/html"});
    		response.write("<html><head> <title>Some fancy website</title></head><body>Sorry. This login (" + docs[0]["login"] +  ") is already taken <form action=\"http://localhost:8887\" method=\"post\">Login:<br><input type=\"text\" name=\"login\"><br>Password:<br><input type=\"password\" name=\"psw\"><br><input type=\"radio\" name=\"type_in\" value=\"signin\">Sign-in<br><input type=\"radio\" name=\"type_in\" value=\"login\">Log-in<br><input type=\"submit\" value=\"Signin\"></form><br><a href=\"http://localhost:8887/twitter\">Sign in with Twitter</a></body></html>");
    		response.end();
  		} else {
  			var entry = {};
			entry["login"]=post["login"];
          ...
          + post["psw"] + ". Welcome!<br><form action=\"http:\/\/localhost:8887\/quit\" method=\"get\"><input type=\"submit\" value=\"Logout\"></form></body></html>");
    				response.end();
